...
[Chrome Cordova App](https://github.com/MobileChromeApps/mobile-chrome-apps) should be install globaly `npm install -g cca`.

The native build requires the `DOMAIN` environment variable be set so that request This 
this is typically done in the user's `local.env.js`.

To prepare files run `grunt cca`. The app can be built and run from within the `cca`
directory using all of the built in `cca` commands.

For the most part all of the files in the `cca` directory are generated by `grunt cca`.
The Chrome App manifests are copied from `app/client`.
